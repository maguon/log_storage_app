...
export const retrieve = () => async (dispatch, getState) => {
    dispatch({ type: actionTypes.retrievePasswordActionTypes.Retrieve_WAITING, payload: {} })
    ...
    try {
        ...
        if (res.success) {
            ...
            dispatch({ type: actionTypes.retrievePasswordActionTypes.Retrieve_SUCCESS, payload: {} })
            Actions.pop()
        } else {
            ToastAndroid.showWithGravity(`密码修改失败：${res.msg }！`, ToastAndroid.CENTER, ToastAndroid.BOTTOM)
            dispatch({ type: actionTypes.retrievePasswordActionTypes.Retrieve_FAILED, payload: { failedMsg: res.msg } })
        }
    } catch (err) {
        ToastAndroid.showWithGravity(`密码修改失败：${res.msg }！`, ToastAndroid.CENTER, ToastAndroid.BOTTOM)
        dispatch({ type: actionTypes.retrievePasswordActionTypes.Retrieve_ERROR, payload: { errorMsg: err } })
    }
}