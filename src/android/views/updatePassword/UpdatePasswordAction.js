    ...
    if (newPassword == confirmPassword) {
        try {
            ...
            if (res.success) {
                ...
                dispatch({ type: actionTypes.updatePasswordActionTypes.change_Password_success, payload: {} })
            } else {
                ToastAndroid.showWithGravity(`修改失败！${res.msg}`, ToastAndroid.CENTER, ToastAndroid.BOTTOM)
                dispatch({ type: actionTypes.updatePasswordActionTypes.change_Password_failed, payload: { failedMsg: res.msg } })
            }
        } catch (err) {
            ToastAndroid.showWithGravity(`修改失败！${err}`, ToastAndroid.CENTER, ToastAndroid.BOTTOM)
            dispatch({ type: actionTypes.updatePasswordActionTypes.change_Password_error, payload: { errorMsg: err } })
        }
    } else {
